...
class GymRolloutController:
    ...
    def generate(self, rollout_id):
        ...
        if self.args.load_debug_rollout_data:
            ...
            data, memory_tool_times, success_rate = self.generate_rollout(self.args, rollout_id, self.data_source, evaluation=False)
        ...
        # 记录日志
        log_rollout_data(rollout_id, self.args, data, memory_tool_times, success_rate, time() - start_time)
        
        # 转换为训练数据
        # data = self._convert_messages_to_train_data(data)
        data = self._convert_tokens_to_train_data(data)
    ...
    def _convert_tokens_to_train_data(self, samples: Union[list[MySample], list[list[MySample]]]):
        ...
        if samples and any(samp.rollout_log_probs for samp in samples):
            ...
            for sample, mask in zip(samples, loss_mask):
                try:
                    ...
                    print(f"Convert to train data failed: {e}")
                    breakpoint()
...
def log_rollout_data(rollout_id, args, samples, memory_tool_times, success_rate, rollout_time):
    ...
    if samples and samples[0].metadata:
        ...
        for sample in samples:
            ...
            if "context_modified" in sample.metadata:
                if sample.metadata["context_modified"]:
                    context_modified_traj.add(sample.metadata["trajectory_id"])
        
        log_dict["rollout/num_trajectories"] = len(trajectory_ids)
        log_dict["rollout/num_prompt_groups"] = len(prompt_group_ids)
        log_dict["rollout/avg_context_modification_times"] = len(context_modified_traj) / len(trajectory_ids)
        log_dict["rollout/memory_tool_times"] = memory_tool_times
        log_dict["rollout/success_rate"] = success_rate
    ...
    if args.use_wandb:
        # 修改：现在每个rollout对应一个训练step
        # Lynx: multiple log of rollout/step
